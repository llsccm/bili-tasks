...
3. **登录并设置 Cookie**
   - **扫码登录（推荐）**：在青龙面板中运行 `login.mjs`，扫描控制台输出的二维码。登录成功后，脚本会自动将 Cookie 添加到青龙环境变量 `BILI_TASK_COOKIES` 中
   - **手动设置**：如果不方便扫码，也可以手动获取 Bilibili 的完整 Cookie，并在青龙环境变量中添加 `BILI_TASK_COOKIES`，值为你的 Cookie
